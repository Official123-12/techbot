import express from "express";
  ...
  app.get(["/services/bots/:slug", "/services/bots/:slug/*"], async (req, res, next) => {
    ...
    try {
      ...
      const name = (template as unknown as { name?: string }).name || "Toxic Bot";
      const imageUrl = (template as unknown as { imageUrl?: string }).imageUrl || "https://i.imgur.com/8kQmQKq.png";
      const host = req.get("host") || "hosting.toxicx.tech";
      const url = `https://${host}/services/bots/${req.params.slug}`;
      const title = `${name} — Deploy on Toxic Host`;
      const desc = `Deploy ${name} bot instantly on Toxic Host. Fast, reliable WhatsApp bot hosting in seconds.`;
  ...
  async function seedDefaultTemplate() {
    try {
      ...
      await BotTemplate.create({
        name: "Toxic-MD",
        githubRepo: "xhclintohn/Toxic-MD",
        sessionIdUrl: "https://toxicx.tech/pairing",
        costTx: 0,
        isDefault: true,
        active: true,
        shareableSlug: "toxic-md"
      });
      logger.info("Seeded default ToxicMD bot template");
    } catch (err) {
  ...
  async function fixBotTemplateImages() {
    try {
      const result = await BotTemplate.updateMany(
        ...
        { $set: { imageUrl: "https://raw.githubusercontent.com/xhclintohn/Music-Clips-Collection/main/images/toxicx.png" } }
      );
